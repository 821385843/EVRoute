...
static NSString * const EV_ROUTER_WILDCARD_CHARACTER = @"~";

NSString *const EVRouteParameterURL = @"EVRouteParameterURL";
NSString *const EVRouteParameterCompletion = @"EVRouteParameterCompletion";
NSString *const EVRouteParameterUserInfo = @"EVRouteParameterUserInfo";
...
@implementation EVRoute

+ (instancetype)sharedIsntance
{
    static EVRoute *instance = nil;
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(void))completion
{
    ...
    if (parameters) {
        EVRouteHandler handler = parameters[@"block"];
        if (completion) {
            parameters[EVRouteParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[EVRouteParameterUserInfo] = userInfo;
        }
        if (handler) {
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url
{
    ...
    parameters[EVRouteParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:EV_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (void)addURLPattern:(NSString *)URLPattern andHandler:(EVRouteHandler)handler
{
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if ((pathSegments.count == 2 && ((NSString *)pathSegments[1]).length) || pathSegments.count < 2) {
            [pathComponents addObject:EV_ROUTER_WILDCARD_CHARACTER];
        }
